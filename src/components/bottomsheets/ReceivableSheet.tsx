...
export const ReceivableSheet = forwardRef<any, { onClose: () => void }>((props, ref) => {
  const receivableParties = parties.filter(p => p.netBalance > 0);
  const { total } = accountSummary.receivable;
  ...
  return (
    ...
    >
      <View className="px-4 pt-2 pb-2 flex-row justify-between items-center">
        <Text className="text-[17px] font-[700] text-text1">Receivable</Text>
      ...
      <View className="px-4 mb-4">
        <Text className="text-[12px] font-[500] text-text3 uppercase mb-1">Total Owed to You</Text>
        <Text className="text-[22px] font-[800] text-green">{formatINR(total)}</Text>
      </View>
      ...
      <BottomSheetScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 40 }}>
            ...
            <View key={party.id} className="bg-surface border border-border rounded-[14px] p-4 mb-3 shadow-sm">
              <View className="flex-row items-center mb-4">
                ...
                <View className="flex-1">
                  <View className="flex-row justify-between items-center">
                    ...
                    <Text className="text-[15px] font-[800] text-green">{formatINR(party.netBalance)}</Text>
                  </View>
              ...
              <View className="flex-row gap-2">
                <Pressable className="flex-1 bg-blue h-10 rounded-[8px] items-center justify-center">
                  <Text className="text-[14px] font-[600] text-white">Record Receipt</Text>
                </Pressable>
        ...
        <Pressable className="mt-2 h-[56px] border border-dashed border-border bg-surface2 rounded-[12px] items-center justify-center">
          <Text className="text-[15px] font-[600] text-blue">+ Add Receivable</Text>
        </Pressable>
